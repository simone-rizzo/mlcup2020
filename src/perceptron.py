...
class perceptron(BaseEstimator, ClassifierMixin):
    def __init__(self, hidden_units, activation, epochs, ETA, ALPHA, LAMBDA, earlyStopping=False, weightInitialization="xav", tolerance=1e-3, patience=None,loss="MEE", regression=False):
        self.ETA = ETA
        self.ALPHA = ALPHA
        self.LAMBDA = LAMBDA
    ...
    def createModel(self):
        ...
        if self.weightInitialization=='xav':
            ...
            return{
                'Wih':np.zeros((self.inputUnits,self.hidden_units)),
                'Who':np.zeros((self.hidden_units,self.outputUnits)),
                'bi':np.zeros((1,self.hidden_units)),
